/**
 * The AutoCAD Color Index palette.
 *
 * Index 0 is BYBLOCK and 256 is BYLAYER; neither is a real colour, so callers
 * must resolve those before looking a value up here. Index 7 is the "foreground"
 * colour, which AutoCAD paints white on a dark background and black on a light
 * one — {@link aciToRgb} handles that inversion.
 */
const ACI: readonly number[] = [
...
/** BYBLOCK: the entity takes the colour of the block reference that draws it. */
export const ACI_BYBLOCK = 0;
/** BYLAYER: the entity takes the colour of its layer. */
export const ACI_BYLAYER = 256;

/**
 * Resolves an AutoCAD colour index to 0xRRGGBB.
 *
 * @param darkBackground when false, index 7 resolves to black instead of white.
 */
export function aciToRgb(index: number, darkBackground: boolean): number {
  // Negative indices mean the layer is off; the caller decides whether to draw,
  // but the magnitude still carries the colour.
  const i = Math.abs(index | 0);
  if (i === 7 || i === 0) return darkBackground ? 0xffffff : 0x000000;
  if (i < 0 || i > 255) return darkBackground ? 0xffffff : 0x000000;
  return ACI[i];
}

/**
 * Picks the colour for an entity, following AutoCAD's resolution order:
 * an explicit true colour wins, then a concrete ACI, then BYLAYER/BYBLOCK which
 * defer to the layer or to the enclosing block's colour.
 *
 * @returns 0xRRGGBB, or -1 to mean "inherit from the layer at render time".
 */
export function resolveEntityColor(
  entity: { color?: number; colorIndex?: number },
  darkBackground: boolean,
  blockColor: number
): number {
  const idx = entity.colorIndex;

  // libredwg reports a packed 24-bit true colour when the entity carries one.
  // A true colour of 0 is indistinguishable from "unset" in this field, so it is
  // only trusted when the index is not one of the inheritance sentinels.
  if (idx !== undefined && idx !== ACI_BYLAYER && idx !== ACI_BYBLOCK) {
    if (typeof entity.color === 'number' && entity.color > 0 && entity.color <= 0xffffff) {
      return entity.color;
    }
    return aciToRgb(idx, darkBackground);
  }

  if (idx === ACI_BYBLOCK) return blockColor;
  // BYLAYER, or no colour information at all.
  return -1;
}

/** Formats 0xRRGGBB as a CSS hex string. */
